        ...
        let animations:(() -> Void) = {[weak self]() in
                ...
                if self.type == .alert {
                    self.snp.updateConstraints({ (make) in
                        make.centerY.equalTo((self.attachedView.snp.centerY)).offset(-100)
                    })
                      self.layoutIfNeeded()
                }
//            if self.type == .sheet {
//                self.snp.updateConstraints({ (make) in
//                    make.centerY.equalTo((self.attachedView.snp.centerY)).offset(-600)
//                })
//                self.layoutIfNeeded()
//            }
        }
...
//                strongSelf.bottomLayoutConstraint.constant  = -350
